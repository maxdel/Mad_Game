...
public class WaterballView extends GameObjectSolidView {

    private ParticleSystem ps;

    public WaterballView(GameObject fireball) {
        super(fireball);
        animation = ResourceManager.getInstance().getAnimation("empty");
        try {
            ps = ParticleIO.loadConfiguredSystem("/res/particles/waterball.xml");
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
